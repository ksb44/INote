...
useNotesRouter.route('/addNotes').post(verifyJWT,async(req,res)=>{
    ...
    try {
        ...
        let note=await Note.create({title,description,tag, userId: req.user._id })
...
useNotesRouter.route("/updateNote/:id").put(verifyJWT , async (req,res)=> {
    ...
    const note = await Note.findByIdAndUpdate(id, { title, description, tag },{new:true});
    if (!note) return res.status(404).send("No note with this id was found.");
    if(note.userId.toString()!==req.user._id.toString())return res.status(403).send("You are not authorized to perform this action");
    res.send(note);
});
useNotesRouter.route( "/deleteNote/:id" ).delete( verifyJWT , async (req,res) => {
    const note = await Note.findByIdAndDelete(req.params.id);
    if (!note) return res.status(400).send("The note with the given ID was not found.")
    if(note.userId.toString()!==req.user._id.toString())return res.status(403).send("You are not authorized to perform this action");
